...
**▶ Resume here — finish the iPhone Shortcut on another machine (handoff 2026-05-24):** The capture API is live and the Chrome extension works end-to-end; only the on-device iPhone Shortcut remains. On the Mac that has your iPhone's Continuity / Universal Clipboard:
1. **Get the latest:** `git pull origin main` (first time on that machine: `git clone https://github.com/intrater/read-watch-listen.git`).
2. **Get the token onto that machine** (it's gitignored — it never travels in the repo): `cd apps/api && vercel link` (choose the `rwl-api` project) → `vercel env pull .env.local`. `CAPTURE_TOKEN_IOS` is type *Encrypted* so it pulls fine. (The Neon `DATABASE_URL` is *Sensitive* and comes back empty — expected, not needed for the Shortcut.)
3. **Build the Shortcut the easy way:** open the **macOS Shortcuts app** (big screen + mouse) and follow `ios/rwl-capture.shortcut.md`, pasting the token from your clipboard. A Shortcut built on the Mac syncs to your iPhone's share sheet via iCloud — much easier than building on the phone. *(Or ask Claude on that machine to generate an importable `.shortcut` file with the token baked in.)*
4. **The contract:** `POST https://rwl-api.vercel.app/api/capture`, header `Authorization: Bearer <CAPTURE_TOKEN_IOS>`, JSON body `{ "url": <shared URL>, "note": <optional>, "source": "ios-shortcut" }`.
5. **Verify:** share a link from Safari/X → ask Claude there to check the DB for a `source='ios-shortcut'` row. That closes U3. Open work after: **U4** (LLM assist — needs the Anthropic key) and **U13** (seed the corpus from your existing X bookmarks — John's stated priority; worth pulling forward from Phase D).
