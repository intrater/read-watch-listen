...
**Status:** ✅ Shipped 2026-05-24. **Chrome extension verified end-to-end against production** (commit `53f8e2f`) — John loaded the unpacked MV3 extension, configured the token in its settings, and saved a real article (Politico AI-EO piece) that synced to Shiori with a live `shiori_id`. **iOS capture path changed 2026-05-24 (handoff session):** the custom `Save to RWL` Shortcut was abandoned mid-build after the inline "why" note (the only reason to wrap Shiori on the iOS side) was punted as not-worth-it for v1. iOS now uses **Shiori's own published Shortcut** (`https://www.shiori.sh/Shiori.shortcut`) which POSTs directly to Shiori — no token from us, no DB row at capture, just a bookmark in Shiori. The hand-rolled assembly spec (`ios/rwl-capture.shortcut.md`) and the programmatic plist builder (`ios/build_shortcut.py`, `ios/Save to RWL.shortcut`) are kept in-tree as reference for if we ever re-add inline notes.

**▶ Resume here — install Shiori's iOS Shortcut (handoff 2026-05-24, take 2):**
1. **On the iPhone, get the Shiori shortcode:** open Shiori (web or app) → iOS Shortcut setup dialog → copy the shortcode (this is the per-account auth token that goes into the Shortcut's `x-shiori` header; not the same value as our `SHIORI_TOKEN` Bearer token but functionally the same role).
2. **Download the Shortcut:** on the iPhone, visit `https://www.shiori.sh/Shiori.shortcut` in Safari → it downloads → tap it to open in Shortcuts → tap **Add Shortcut**.
3. **Paste the shortcode:** edit the imported Shortcut → find the `x-shiori` header on the last action → paste the shortcode → save.
4. **Add to share-sheet favorites** for quick access.
5. **Verify:** share a URL from Safari → pick **Shiori** → confirm it lands in Shiori. That closes the iOS half of U3.
...
- **First step (added 2026-05-24): Shiori → Postgres sync.** iOS captures land directly in Shiori (Shiori's own Shortcut, not our `/capture` endpoint — see U3's path change). Before composition, the cron calls `GET /api/links?since=<last_sync_at>` on Shiori and upserts any Shiori-only rows into `captures` (source `shiori-sync`, `shiori_status='synced'`, `note=null`, `rwl_tag` from URL pattern, `consume_minutes=null`). Chrome-extension captures already exist as Postgres rows from `/api/capture`, so the upsert is a no-op for them. Stores `last_sync_at` in `kv_state`.
